...
public class FileMovieInfoVirtualFolder extends VirtualFolder {
	...
	public void resolve() {
		...
		if (!resolved && children.size() == 0) {
			getConfig();
			MovieInfoVirtualFolder fld = null;
			if (parent.getParent().getParent().getName().contains("[DVD ISO]"))
				isDVD = true;
...
//				System.out.println(plugin.getClass().getSimpleName() + " nfo file not found, search filename");
				String moviename = "";
				if (!isDVD)
					moviename = getMovienameFromFilename(parent.getName());
				else
					moviename = getMovienameFromFilename(parent.getParent()
							.getParent().getDisplayName().replace("[DVD ISO] ", ""));
				//System.out.println("Moviename = " + moviename);
//				System.out.println(plugin.getClass().getSimpleName() + " Moviename = " + moviename);
				if (!moviename.equals(""))
					nfo = plugin.lookForMovieID(getWebsite(moviename, null,
							GOOGLE, plugin.getGoogleSearchSite()));
				if (nfo != null)
					{
//					System.out.println(plugin.getClass().getSimpleName() + " nfo =" +nfo);
					plugin.importFile(getWebsite(nfo, plugin.getVideoURL(),
							MOVIEWEBSITE, null));
					}
				else {
//					System.out.println(plugin.getClass().getSimpleName() + " movie from moviename not found, search directory name");
					if (!isDVD)
						moviename = getMovienameFromFilename(getMovienameFromFilename(parent.getParent().getParent()
								.getName()));
					else moviename = getMovienameFromFilename(parent.getParent().getParent().getParent().getName());
			...
			if (nfo != null) {
				for (int i = 0; i < disp.length; i++) {
					...
					if(disp[i].equals("rating")){
						rating = plugin.getRating();displayRating(fld);}
					if(disp[i].contains("genre")){
						genre = plugin.getGenre();displayGenre(fld);}
					if(disp[i].contains("plot")){
						plot = plugin.getPlot();displayPlot(fld);}
					if(disp[i].contains("director")){
						dir = plugin.getDirector();displayDirector(fld);}
					if(disp[i].contains("cast")){
						castlist = plugin.getCast();displayCast(fld);}
					if(disp[i].contains("tagline")){
						tagline = plugin.getTagline();displayTagline(fld);}
					if(disp[i].equals("agerating")){
						agerating = plugin.getAgeRating();displayAgeRating(fld);}
				}
...
//			  System.out.println("thumb: " + thumb);
	...
	private void displayTitle(MovieInfoVirtualFolder fld){
		if (title != null) {
			...
			if (className.equals("IMDB")) {
				String tag = "[HD] ";
			/*	//String yt = getYoutubeTrailer(getWebsite(title.replace(" & ", " ")  + " trailer hd", null,GOOGLE, "youtube.com"));
				String[] yt = getYoutubeTrailer(getWebsite(title.replace(" & ", " ")  + " trailer", 
						"http://www.youtube.com/results?search_type=videos&search_query=###MOVIEID###&high_definition=1"
						,MOVIEWEBSITE, null),true);
				fld.addChild(new MovieInfoTrailer(tag + yt[1], thumb,yt[0]));
				...
				//fld.addChild(new MovieInfoTrailerDirectStream("Trailer Direct Stream: " + title, thumb.replaceAll("S[X,Y][0-9]{2,3}_S[X,Y][0-9]{2,3}_", "SX300_SY300_"),yt));*/
				if(trailer!=null) {
					fld.addChild(new WebVideoStream("Trailer",trailer,thumb));
				}
			}
			addChild(fld);
...
//			System.out.println("Plot clean = " + plot);
	...
	private String imdbIDFromNfo(String movieName) {
		...
		try {
			...
			if (!isDVD)
			f = new File((parent.getParent().getParent().getSystemName()
					+ "\\" + parent.getName()).replaceAll("\\..{2,4}$", ".nfo"));	
			else 
				f = new File((parent.getParent().getParent().getParent().getSystemName()
						+ "\\" + parent.getParent().getParent().getName())
						.replace("[DVD ISO] ", "").replaceAll("\\{.*\\}","").replaceAll("\\..{2,4}$", "") + ".nfo");
			if (f.exists()) {
				fr = new FileReader(f);
...
//				System.out.println("movie name for nfo search" + movieName);
				URL url = new URL(
						"http://www.newzleech.com/?group=&minage=&age=&min=min&max=max&q="
								+ movieName + "&m=search&adv=1");
	...
	private String lookForImdbID(BufferedReader in) {
		try {
			...
			else
			{
				...
				if(pluginUrl.contains("csfd.cz") && newURL.matches("^.*[0-9]{4,}/{0,1}$"))
				{
					if(newURL.endsWith("/"))
	...
	private BufferedReader getWebsite(String mname,String url, boolean google, String searchURL) {
		...
		try {
			...
			if (!google){
				...
				mname = mname.replace(" ", "+");
				u = new URL(("http://www.google.com/search?hl=en&q=" + mname + "+site%3A" + searchURL));
			}
...
//			System.out.println("host= " + host);
			...
			else conn.setRequestProperty("Host", "");
...
/*			if(cookie!=null&&cookie.length()!=0)
	...
	private void getConfig() 
	{
		...
		if (miConf.exists()) {
			try {
				...
				String line = null;filter = "";String info = "";cover = "";
				if(!cleanlist.isEmpty())cleanlist.clear();			
				while ((line=br.readLine()) != null) {
					line = line.trim();
					if (line.length() > 0 && !line.startsWith("#") && line.indexOf("=") > -1) { 
						if(line.startsWith("Filter="))filter += line.substring(line.indexOf("=")+2,line.lastIndexOf("\"")).toUpperCase();
						if(line.startsWith("Plugins="))
							if(line.indexOf(",")>-1)
							priority = line.substring(line.indexOf("=")+1,line.indexOf(",")).toUpperCase();
							else priority = line.substring(line.indexOf("=")+1,line.length()).toUpperCase();
	...
	protected String getMovienameFromFilename(String displayname) {
		...
		Pattern a;
		String[] pat = { ".[Ss][0-9]{1,2}[Ee][0-9]{2}", // Tv show with episode
				// s01e01
				"[^aA0-zZ9][0-9]{3}[^aA0-zZ9]", // TV show with episode 313
				"[^0-9][\\(,\\]]{0,1}[0,1,2][0,9][0-9]{2}[\\),\\]]{0,1}" }; // video with year
		String f = null;
		for (int i = 0; i < pat.length; i++) {
			...
			h = " " + h + " ";
			h = h.replaceAll("[^aA0-zZ9á]", " ");
			h = h.replaceAll("\\p{Punct}", " ");
			h = h.replaceAll("\\s{2,}"," ");
			h = h.toUpperCase();
			...
			h = h.trim();
			if(f!=null)
			if(f.contains("сериал"))h = h + " %D1%81%D0%B5%D1%80%D0%B8%D0%B0%D0%BB";
//		System.out.println("getMovienameFromFilename Returns " + h);
	...
	public String clean(String s) {
		...
		while (ma.find()) {
			tmp = ma.group();
			if (tmp.contains("x"))
				s = s.replaceAll(tmp, (char) Integer.valueOf(
						tmp.replaceAll("[^0-9a-fA-F]", ""), 16).intValue()
						+ "");
				else s = s.replaceAll(tmp, (char) Integer.valueOf(
						tmp.replaceAll("[aA-zZ\\W]", "")).intValue()
						+ "");
		}
	...
	public FileMovieInfoVirtualFolder(String name, String thumbnailIcon,int act, int line,String nfo) {
		super(name, thumbnailIcon);
		if(act > 0)numberOfActors = act;
		if(line >0)lineLength = line;
		className = name;
		nfoId=nfo;
	}
